...
export class ProductCreateEditComponent implements OnInit {
  ...
  public onSubmit( productForm: FormGroup ) {
    ...
    // Если есть id, то данные мы взяли с сервера - значит редактируем
  ...
  // Редактировать продукт или создать новый
      ...
      if ( id ) {
        this.service.getProduct(id).subscribe(
          product => {
            ...
            this.productForm.patchValue(this.currentProduct); // замена всех значений, которые находятся в форме
          },
          error => this.errorMessage = error
        );
      }
